...
func setRoute(iface string, addr *net.IPNet) error {
	var err error
	_, err = ncutils.RunCmd("route ADD "+addr.String()+" "+addr.IP.String(), false)
	time.Sleep(time.Second >> 2)
	ncutils.RunCmd("route CHANGE "+addr.IP.String()+" MASK "+addr.Mask.String()+" "+addr.IP.String(), false)
	return err
}
...
func deleteRoute(iface string, addr *net.IPNet) error {
	var err error
	_, err = ncutils.RunCmd("route DELETE "+addr.IP.String()+" MASK "+addr.Mask.String()+" "+addr.IP.String(), false)
	return err
}

func setCidr(iface string, addr *net.IPNet) {
	ncutils.RunCmd("route ADD "+addr.String()+" "+addr.IP.String(), false)
	time.Sleep(time.Second >> 2)
	ncutils.RunCmd("route CHANGE "+addr.IP.String()+" MASK "+addr.Mask.String()+" "+addr.IP.String(), false)
