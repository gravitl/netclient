...
func SetExplicitRoute(iface string, destination *net.IPNet, gateway string) error {
	return setRoute(iface, destination)
}
...
func setCidr(iface string, addr *net.IPNet) {
	if iplib.Version(addr.IP) == 4 {
		ncutils.RunCmd("route -q -n add -net "+addr.String()+" "+addr.IP.String(), false)
	} else if iplib.Version(addr.IP) == 6 {
		ncutils.RunCmd("route -A inet6 -q -n add -net "+addr.String()+" "+addr.IP.String(), false)
	} else {
