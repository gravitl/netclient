...
// IMPORTANT: Must remove jump rules BEFORE deleting chains to avoid GetRules errors
func (n *nftablesManager) removeVirtualNATRules(egressID string) error {
	...
	// This prevents GetRules errors when target chains don't exist
	preroutingBase, err := n.getChain(defaultNatTable, "PREROUTING")
	if err == nil {
		jumpKey := genRuleKey("jump", preroutingChain, "vnat", id8)
		rules, err := n.conn.GetRules(natTable, preroutingBase)
		if err != nil {
			slog.Debug("failed to get PREROUTING rules for VNAT cleanup", "error", err)
		} else {
			for _, r := range rules {
				if string(r.UserData) == jumpKey {
					if err := n.conn.DelRule(r); err != nil {
						slog.Warn("failed to delete VNAT prerouting jump rule", "error", err)
					} else {
						slog.Info("removed VNAT prerouting jump rule", "chain", preroutingChain)
					}
					break
				}
			}
	...
	if err == nil {
		jumpKey := genRuleKey("jump", postroutingChain, "vnat", id8)
		rules, err := n.conn.GetRules(natTable, postroutingBase)
		if err != nil {
			slog.Debug("failed to get POSTROUTING rules for VNAT cleanup", "error", err)
		} else {
			for _, r := range rules {
				if string(r.UserData) == jumpKey {
					if err := n.conn.DelRule(r); err != nil {
						slog.Warn("failed to delete VNAT postrouting jump rule", "error", err)
					} else {
						slog.Info("removed VNAT postrouting jump rule", "chain", postroutingChain)
					}
					break
				}
			}
	...
	// Step 2: Flush to ensure jump rules are removed before deleting chains
	if err := n.conn.Flush(); err != nil {
		slog.Warn("failed to flush after removing VNAT jump rules", "error", err)
	}

	// Step 3: Now safe to delete per-egress chains (jump rules are already removed)
	if err := n.deleteVNATChain(preroutingChain); err != nil {
		slog.Warn("failed to delete VNAT prerouting chain", "chain", preroutingChain, "error", err)
	}
	if err := n.deleteVNATChain(postroutingChain); err != nil {
		slog.Warn("failed to delete VNAT postrouting chain", "chain", postroutingChain, "error", err)
	}

	// Final flush to ensure all changes are applied
	if err := n.conn.Flush(); err != nil {
		return fmt.Errorf("failed to flush after removing VNAT rules: %w", err)
	}

	slog.Info("removed virtual NAT rules", "egressID", egressID)
	return nil
}
